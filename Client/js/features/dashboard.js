...
export const DashboardFeature = {
  ...
  updatePerformanceStats(perf) {
    ...
    if (
      elRamAbs &&
      perf.ramUsedGB !== undefined &&
      perf.ramTotalGB !== undefined
    ) {
      elRamAbs.textContent = `${Number(perf.ramUsedGB).toFixed(1)} / ${Number(
        perf.ramTotalGB
      ).toFixed(1)} GB`;
    } else if (elRamAbs) {
    ...
    if (perf.diskUsedGB !== undefined && perf.diskTotalGB !== undefined) {
      ...
      if (elSsdAbs)
        elSsdAbs.textContent = `Used ${used.toFixed(1)} / ${total.toFixed(
          1
        )} GB`;
